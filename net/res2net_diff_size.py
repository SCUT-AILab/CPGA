...
def res2net50_26w_6s_with_Dis(pretrained=False, **kwargs):
    """Constructs a Res2Net-50_26w_4s model with discriminator.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = Res2Net_with_Dis(Bottle2neck, [3, 4, 6, 3], baseWidth=26, scale=6, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['res2net50_26w_6s']))
    return model


class Res2Net_with_Dis(Res2Net):
    def __init__(self, block, layers, baseWidth=26, scale=4, num_classes=1000, D_lambda=0.1):
        super(Res2Net_with_Dis, self).__init__(block, layers, baseWidth, scale, num_classes)
        self.Discriminator = Discriminator(D_lambda)
    ...
    def forward(self, x):
        ...
        y_classification = self.fc(feature_map)
        domain_discrimination = self.Discriminator(feature_map)

        return domain_discrimination, y_classification


class Discriminator(nn.Module):
    """feature Discriminator for different domains"""
    def __init__(self, D_lambda=0.1):
        super(Discriminator, self).__init__()
        self.GRL = GRL(Lambda=D_lambda)
...
class Res2Net_for_2stream(nn.Module):

    def __init__(self, block, layers, baseWidth=26, scale=4, num_classes=1000):
        self.inplanes = 64
        super(Res2Net_for_2stream, self).__init__()
        self.baseWidth = baseWidth
        self.scale = scale
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
...
class Res2Net_with_2stream(nn.Module):
    def __init__(self, block, layers, baseWidth=26, scale=4, num_classes=1000):
        super(Res2Net_with_2stream, self).__init__()
        self.stream_semi = Res2Net_for_2stream(block, layers, baseWidth=baseWidth, scale=scale, num_classes=1000)
        self.stream_all = Res2Net_for_2stream(block, layers, baseWidth=baseWidth, scale=scale, num_classes=1000)
        self.fc = nn.Linear(512 * block.expansion, num_classes)

    def forward(self, semix, x):
        x_semi = self.stream_semi(semix)
        x_all = self.stream_all(x)
        x = x_semi + x_all
        out = self.fc(x)
        return out

def res2net50_26w_6s_2stream(pretrained=False, **kwargs):
    """Constructs a Res2Net-50_26w_4s model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = Res2Net_with_2stream(Bottle2neck, [3, 4, 6, 3], baseWidth=26, scale=6, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['res2net50_26w_6s']))
    return model
